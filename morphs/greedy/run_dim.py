...
##############################################################################
#
# Code for constructed dataset of faces to morph
#
##############################################################################


class MorphDataset(torch.utils.data.Dataset):
    def __init__(self, morphs_list, image_size, return_opencv=False):
        self.image_size = image_size
        self.files = []
        for stype in ["train", "test"]:
            mdir = f"/cluster/nbl-users/Shreyas-Sushrut-Raghu/FaceMoprhingDatabases/PRINT_SCAN/digital/morph/lma/{stype}"
            mimages = [
                file.split(".")[0].split("-vs-")
            ...
            train_dir = f"/cluster/nbl-users/Shreyas-Sushrut-Raghu/FaceMoprhingDatabases/PRINT_SCAN/digital/aligned/{stype}/"
            outdir = f"/cluster/nbl-users/Shreyas-Sushrut-Raghu/FaceMoprhingDatabases/PRINT_SCAN/digital/morph/greedy/{stype}"
            ...
            for row in mimages:
                ...
                img2 = img2.split("d")[0]
                file1, file2, ofile = (
                    os.path.join(train_dir, img1 + "-1.png"),
                    os.path.join(train_dir, img2 + "-1.png"),
                    os.path.join(
                        outdir,
                        row[0].split(".")[0] + "-vs-" + row[1].split(".")[0] + ".png",
                    ),
                )
                if os.path.isfile(file1) and os.path.isfile(file2):
                    self.files.append((file1, file2, ofile))
...
parser.add_argument(
    "config", type=str, help="YAML file containing configuration for the run"
)
...
if "greedy" in config:
    ...
    elif config["loss_fn"]["type"] == "zhang_plus_perceptual":
        perceptual_model = MorphLPIPS(net="vgg")

        def loss_fn(x, y, z):
            id_loss = zhang_identity_prior(loss_model, x, y, z)
            perceptual_loss = 0.5 * (loss_model(x, y) + loss_model(x, z))

    else:
        raise Exception("Invalid `loss_fn` config!")


dataset = MorphDataset("", conf.img_size)
