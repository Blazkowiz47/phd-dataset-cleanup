    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        if config["backbone_type"].startswith("resnet_v2_m"):
            arg_sc = modifiedResNet_v2.resnet_arg_scope(
                weight_decay=config["weight_decay"], batch_norm_decay=config["bn_decay"]
            )
        ...
        if config["out_type"] == "E":
            with slim.arg_scope(arg_sc):
                net = slim.batch_norm(
                    net, activation_fn=None, is_training=is_training_bn
                )
                net = slim.dropout(
                    net, keep_prob=config["keep_prob"], is_training=is_training_dropout
                )
                net = slim.flatten(net)
                net = slim.fully_connected(
                    net, config["embd_size"], normalizer_fn=None, activation_fn=None
                )
                net = slim.batch_norm(
                    net, scale=False, activation_fn=None, is_training=is_training_bn
                )
