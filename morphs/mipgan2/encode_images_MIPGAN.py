...
def main():
    ...
    parser.add_argument(
        "--morph_list_CSV",
        default="morph_list.csv",
        help="CSV file containting the morphing pairs with their FILENAME ONLY of each contributing image. Format: /<image1> , /<image2>",
    )
    parser.add_argument(
        "--src_dir",
        default="aligned_images/",
        help="Directory with images for morphing",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--frs_config_path",
        type=str,
        default="./models/frs/configs/config_ms1m_100.yaml",
        help="config path, used when mode is build",
    )
    parser.add_argument(
        "--frs_model_path",
        type=str,
        default="models/frs/config_ms1m_100_1006k/best-m-1006000",
        help="model path",
    )
    parser.add_argument(
    ...
    # parser.add_argument('--model_url', default='gdrive:networks/stylegan2-ffhq-config-f.pkl', help='Fetch a StyleGAN model to train on from this URL') # karras2019stylegan-ffhq-1024x1024.pkl
    parser.add_argument(
        "--model_url",
        default="./models/stylegan2_finetuned_ICAO.pkl",
    ...
    parser.add_argument(
        "--lr", default=0.03, help="Learning rate for perceptual model", type=float
    )
    parser.add_argument(
        "--decay_rate", default=0.95, help="Decay rate for learning rate", type=float
    )
    parser.add_argument(
        "--iterations",
        default=150,
    ...
    parser.add_argument(
        ...
        default=10000000,
        help="Stop after the loss is lower than this threshold. Disable: set a very large threshold",
        type=float,
    )
    parser.add_argument(
        "--early_stopping_ratio",
        default=0,
        help="Stop after the decrease of the loss (loss[-1]-loss[-1]) has reached the threshold. Disable: 0 ",
        type=float,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--load_resnet",
        default="models/resnet_18_20191231.h5",
    ...
    parser.add_argument(
        "--use_perceptual_loss",
        default=0.0002,
        help="Use VGG multi-layer perceptual loss; 0 to disable, > 0 to scale.",
        type=float,
    )
    ...
    parser.add_argument(
        "--use_pixel_loss_mse",
        default=0,
        help="Use mse image pixel loss; 0 to disable, > 0 to scale.",
        type=float,
    )
    parser.add_argument(
        "--use_msssim_loss",
        default=1,
        help="Use MS-SSIM perceptual loss; 0 to disable, > 0 to scale.",
        type=float,
    )
    ...
    parser.add_argument(
        "--use_arcface_loss",
        default=10,
        help="Use arcface to attack frs system",
        type=float,
    )
    parser.add_argument(
        "--use_arcface_difference_loss",
        default=1,
        help="Constrain the same similarity to each reference image",
        type=float,
    )
    # Generator params
    parser.add_argument(
    ...
    parser.add_argument(
        "--clipping_threshold",
        default=0,
        help="Stochastic clipping of gradient values outside of this threshold. Disable: set threshold < 0",
        type=float,
    )
    ...
    for images_batch in tqdm(
        split_to_batches(ref_images, 1), total=len(ref_images) // 1
    ):
        names = []
        name_temp = []
        fileflag = False
        for pair in images_batch:
            for filename in pair:
                if not os.path.isfile(os.path.join(args.src_dir, filename)):
                    fileflag = True
                name_temp.append(filename.split(".")[0])
            names.append("-vs-".join(name_temp))

        if fileflag or os.path.isfile(
            os.path.join(args.generated_images_dir, f"{names[0]}.png")
        ):
            continue
            ...
            if ff_model is not None:  # predict initial dlatents with ResNet model
                if args.use_preprocess_input:
                    dlatents = 0.5 * ff_model.predict(
                        preprocess_input(
                            load_ref_image(
                                os.path.join(args.src_dir, images_batch[0][0]),
                        ...
                        preprocess_input(
                            load_ref_image(
                                os.path.join(args.src_dir, images_batch[0][1]),
                                image_size=args.resnet_image_size,
                            )
                        )
                    )
            ...
            if args.early_stopping:  # early stopping feature
                if prev_loss is not None:
                    if avg_loss is not None:
                        ...
                        if (
                            avg_loss < args.early_stopping_ratio
                            and prev_loss < args.early_stopping_threshold
