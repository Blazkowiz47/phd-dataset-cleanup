...
def generator_output_2_arcface(generator_output_tensor,box,target_img_size=112,):
    temp_tensor=tf.transpose(generator_output_tensor, [0, 2, 3, 1])
    cutted_tensor=tf.slice(temp_tensor, [0, box[0],box[1], 0], [1, box[2], box[3], 3])
    reshaped_tensor=tf.reshape(tf.image.resize_nearest_neighbor(cutted_tensor,(target_img_size, target_img_size), align_corners=True),[1,target_img_size,target_img_size,3])
...
class PerceptualModel:
    ...
    def build_perceptual_model(self, generator, discriminator=None):
        ...
        #################################################
        detector = MTCNN()
        box=detector.detect_faces(generated_image_tensor.eval().reshape((1024,1024,3)))[0]['box']
        box=box2square(box)
        self.generated_image_arcface = generator_output_2_arcface(generator.generator_output,box,112)


        # FRS embeddings for each contributing subjects
        self.ref_embedding_sub1=tf.get_variable('ref_embedding_sub1', shape=(512,),
                                                    dtype='float32', initializer=tf.initializers.zeros())
        self.ref_embedding_sub2=tf.get_variable('ref_embedding_sub2', shape=(512,),
                                                    dtype='float32', initializer=tf.initializers.zeros())
        ...
        embeddings_morph, _ = model.get_embd(self.generated_image_arcface, False, False, self.arcface_config)
        ...
        if self.arcloss_weight is not None:       
            self.arcloss=distance_loss(embeddings_morph,self.ref_embedding_sub1)*0.5+distance_loss(embeddings_morph,self.ref_embedding_sub2)*0.5

        # Identity-Difference loss
        if self.arc_diff_loss_weight is not None:
            self.arc_diff_loss=tf.abs(distance_loss(embeddings_morph,self.ref_embedding_sub1)-distance_loss(embeddings_morph,self.ref_embedding_sub2))
        ...
        self.morph_weight = tf.get_variable('morph_weight', shape=generated_image.shape,
                                               dtype='float32', initializer=tf.initializers.zeros())
        self.add_placeholder("morph_weight")

        self.ref_img1 = tf.get_variable('ref_img1', shape=generated_image.shape,
                                                dtype='float32', initializer=tf.initializers.zeros())
        self.ref_img2 = tf.get_variable('ref_img2', shape=generated_image.shape,
                                               dtype='float32', initializer=tf.initializers.zeros())
        ...
        if (self.perceptual_loss is not None):
            ...
            self.perceptual1_2 = Model(inputs=vgg16.input, outputs=vgg16.get_layer('block1_conv2').output)
            self.perceptual1_2.trainable = False
            img_feature_1_2=self.perceptual1_2(preprocess_input(self.morph_weight * generated_image))

            self.perceptual2_2 = Model(inputs=vgg16.input, outputs=vgg16.get_layer('block2_conv2').output)
            self.perceptual2_2.trainable = False
            img_feature_2_2=self.perceptual2_2(preprocess_input(self.morph_weight * generated_image))

            self.perceptual3_3 = Model(inputs=vgg16.input, outputs=vgg16.get_layer('block3_conv3').output)
            ...
            self.ref_img1_feature_1_1 = tf.get_variable('ref_img1_feature_1_1', shape=img_feature_1_1.shape,
                                                    dtype='float32', initializer=tf.initializers.zeros())
            self.ref_img1_feature_1_2 = tf.get_variable('ref_img1_feature_1_2', shape=img_feature_1_2.shape,
                                                    dtype='float32', initializer=tf.initializers.zeros())
            self.ref_img1_feature_2_2 = tf.get_variable('ref_img1_feature_2_2', shape=img_feature_2_2.shape,
                                                    dtype='float32', initializer=tf.initializers.zeros())
            self.ref_img1_feature_3_3 = tf.get_variable('ref_img1_feature_3_3', shape=img_feature_3_3.shape,
                                                    dtype='float32', initializer=tf.initializers.zeros())
            ...
            self.ref_img2_feature_1_1 = tf.get_variable('ref_img2_feature_1_1', shape=img_feature_1_1.shape,
                                                    dtype='float32', initializer=tf.initializers.zeros())
            self.ref_img2_feature_1_2 = tf.get_variable('ref_img2_feature_1_2', shape=img_feature_1_2.shape,
                                                    dtype='float32', initializer=tf.initializers.zeros())
            self.ref_img2_feature_2_2 = tf.get_variable('ref_img2_feature_2_2', shape=img_feature_2_2.shape,
                                                    dtype='float32', initializer=tf.initializers.zeros())
            self.ref_img2_feature_3_3 = tf.get_variable('ref_img2_feature_3_3', shape=img_feature_3_3.shape,
                                                    dtype='float32', initializer=tf.initializers.zeros())
            ...
            loss_perceptual = (tf_custom_mse_loss(self.ref_img1_feature_1_1,img_feature_1_1) + tf_custom_mse_loss(self.ref_img1_feature_1_2,img_feature_1_2) + tf_custom_mse_loss(self.ref_img1_feature_2_2,img_feature_2_2) + tf_custom_mse_loss(self.ref_img1_feature_3_3,img_feature_3_3)
            + tf_custom_mse_loss(self.ref_img2_feature_1_1,img_feature_1_1) + tf_custom_mse_loss(self.ref_img2_feature_1_2,img_feature_1_2) + tf_custom_mse_loss(self.ref_img2_feature_2_2,img_feature_2_2) + tf_custom_mse_loss(self.ref_img2_feature_3_3,img_feature_3_3))
        ...
        if (self.pixel_loss_logcosh is not None):
            self.loss += self.pixel_loss_logcosh * tf_custom_logcosh_loss(self.ref_img1,generated_image)*0.5
            self.loss += self.pixel_loss_logcosh * tf_custom_logcosh_loss(self.ref_img2,generated_image)*0.5


        # + mse loss on image pixels
        if (self.pixel_loss_mse is not None):
            self.loss += self.pixel_loss_mse * tf_custom_mse_loss(self.ref_img1, generated_image)*0.5
            self.loss += self.pixel_loss_mse * tf_custom_mse_loss(self.ref_img2, generated_image)*0.5

            
        # + MS-SIM loss on image pixels
        if (self.msssim_loss is not None):
            self.loss += self.msssim_loss * tf.math.reduce_mean(1-tf.image.ssim_multiscale(self.ref_img1, generated_image, 1))*0.5
            self.loss += self.msssim_loss * tf.math.reduce_mean(1-tf.image.ssim_multiscale(self.ref_img2, generated_image, 1))*0.5
    ...
    def set_reference_images(self, images_list, name):
        ...
        if self.perceptual_loss is not None:
            ref_img1_feature_1_1=self.perceptual1_1.predict_on_batch(preprocess_input(image_mask*np.array(ref_img1)))
            ref_img1_feature_1_2=self.perceptual1_2.predict_on_batch(preprocess_input(image_mask*np.array(ref_img1)))
            ref_img1_feature_2_2=self.perceptual2_2.predict_on_batch(preprocess_input(image_mask*np.array(ref_img1)))
            ref_img1_feature_3_3=self.perceptual3_3.predict_on_batch(preprocess_input(image_mask*np.array(ref_img1)))            
            ref_img2_feature_1_1=self.perceptual1_1.predict_on_batch(preprocess_input(image_mask*np.array(ref_img2)))
            ref_img2_feature_1_2=self.perceptual1_2.predict_on_batch(preprocess_input(image_mask*np.array(ref_img2)))
            ref_img2_feature_2_2=self.perceptual2_2.predict_on_batch(preprocess_input(image_mask*np.array(ref_img2)))
            ref_img2_feature_3_3=self.perceptual3_3.predict_on_batch(preprocess_input(image_mask*np.array(ref_img2)))

        if image_mask is not None:
            self.assign_placeholder("morph_weight", image_mask)
        if ((ref_img1_feature_1_1 is not None) and (ref_img1_feature_1_2 is not None) and (ref_img1_feature_2_2 is not None) and (ref_img1_feature_3_3 is not None) 
        and (ref_img2_feature_1_1 is not None) and (ref_img2_feature_1_2 is not None) and (ref_img2_feature_2_2 is not None) and (ref_img2_feature_3_3 is not None)):
    ...
    def optimize(self, vars_to_optimize, iterations=200, use_optimizer='adam'):
        ...
        if use_optimizer == 'lbfgs':
            ...
            if self.arcloss is not None and self.arc_diff_loss is not None:
                fetch_ops = [min_op, self.loss, self.learning_rate, self.arcloss, self.arc_diff_loss]
        ...
        for _ in range(iterations):
            if use_optimizer == 'lbfgs':
                ...
                if self.arcloss is not None and self.arc_diff_loss is not None:
                    _, loss, lr, arcface_loss, arcface_difference_loss = self.sess.run(fetch_ops)
                    ...
                    yield {"loss":loss,"lr":lr, "id_loss":self.arcloss_weight*arcface_loss, "id_diff_loss":self.arc_diff_loss_weight*arcface_difference_loss}
                elif self.arcloss is not None:
                    _, loss, lr, arcface_loss = self.sess.run(fetch_ops)
                    # self.list_loss.append(loss)
                    # self.list_lr.append(lr)
                    # self.list_arcface_loss.append(arcface_loss)
                    yield {"loss":loss,"lr":lr, "id_loss":self.arcloss_weight*arcface_loss}
                else:
