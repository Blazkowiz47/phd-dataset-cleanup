...
class Generator:
    def __init__(self, model, batch_size, custom_input=None, clipping_threshold=0, tiled_dlatent=False, model_res=1024, randomize_noise=False):
        self.batch_size = batch_size
        self.tiled_dlatent=tiled_dlatent
        self.model_scale = int(2*(math.log(model_res,2)-1)) # For example, 1024 -> 18
        # rnd = np.random.RandomState(5)
        if tiled_dlatent:
            # self.initial_dlatents = rnd.randn(self.batch_size, 512)
            ...
            # self.initial_dlatents = 0.1*rnd.randn(self.batch_size, self.model_scale, 512)
