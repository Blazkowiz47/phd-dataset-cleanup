...
class DDPM(pl.LightningModule):
    ...
    def __init__(
        ...
        for name, param in self.model.named_parameters():  # NOTE: fintuning todo
            if all(x in name for x in ["attn2", "to_q_id"]):
                param.requires_grad = True
            elif all(x in name for x in ["attn2", "to_v_id"]):
                param.requires_grad = True
            elif all(x in name for x in ["attn2", "to_k_id"]):
                param.requires_grad = True
            else:
                param.requires_grad = True
        ...
        if isinstance(x, dict):
            for k in x.keys():
                if torch.is_tensor(x[k]):
                    if len(x[k].shape) == 3:
                        x[k] = x[k][..., None]
                    x[k] = x[k].to(memory_format=torch.contiguous_format).float()
            return x
        if len(x.shape) == 3:
            x = x[..., None]
    ...
    def get_learned_conditioning(self, c, x=None):
        if self.cond_stage_forward is None:
            if hasattr(self.cond_stage_model, "encode") and callable(
                ...
                c = self.cond_stage_model.encode(c, x=x)
        ...
        if self.model.conditioning_key is not None:
            ...
            if cond_key != self.first_stage_key:
                if cond_key in ["caption", "coordinates_bbox"]:
                    ...
                    # if bs is not None: # might uncomment during training
                    #    for key in xc.keys():
                    #        xc[key] = xc[key][:bs]
    ...
    def forward(self, x, c, *args, **kwargs):
        ...
        if self.model.conditioning_key is not None:
            ...
            if self.cond_stage_trainable:
                c = self.get_learned_conditioning(c, x)
