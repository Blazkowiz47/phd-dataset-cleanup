...
# sys.path.append(os.path.join(current_dir,'src','taming-transformers'))
...
def get_model():
    config_dir = os.path.join(
        "/home/ubuntu/cluster/nbl-users/Haoyu/MA_ladimo/latent-diffusion/configs/latent-diffusion/frgc-ldm-vq-f8.yaml"
    )
    ladimo_model_dir = os.path.join(
        "/home/ubuntu/cluster/nbl-users/Haoyu/MA_ladimo/latent-diffusion/logs/2023-11-15T10-04-11_ffhq-ldm-vq-f8/checkpoints/epoch=000096.ckpt"
    )
...
x1 = torch.from_numpy(
    ldm_data.datasets["train"][first_morph_idx]["image"].reshape((1, 256, 256, 3))
).to("cuda")
x2 = torch.from_numpy(
    ldm_data.datasets["train"][second_morph_idx]["image"].reshape((1, 256, 256, 3))
).to("cuda")

c1 = (
    ldm_data.datasets["train"][first_morph_idx]["fr_embeds"]
    .reshape((1, 1, 512))
    .to("cuda")
)
c2 = (
    ldm_data.datasets["train"][second_morph_idx]["fr_embeds"]
    .reshape((1, 1, 512))
    .to("cuda")
)

# Compute linear interpolation between (this is where the morphing happens)
cm = slerp(c1, c2, 0.5)
...
fname = f"id1-vs-id2.png"
outdir = os.path.join(parent_dir, f"ladimo_output/{fname}")
