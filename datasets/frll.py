...
ROOT_DIR = "/mnt/cluster/nbl-users/Shreyas-Sushrut-Raghu/FaceMoprhingDatabases/cleaned_datasets/frill/raw/"
...
def clean_digital_bon() -> None:
    files = glob(os.path.join(ROOT_DIR, "*_08.jpg"))
    odir = os.path.join(CLEAN_DIR, "digital", BONAFIDE, RAW, "test")
    os.makedirs(odir, exist_ok=True)
    for file in tqdm(files):
        shutil.copy(file, os.path.join(odir, os.path.split(file)[1]))
...
def split_train_test() -> None:
    ...
    train_males = random.sample(maleids, k=int(len(maleids) * 0.7))
    train_females = random.sample(femaleids, k=int(len(femaleids) * 0.7))
        ...
        for sid, info in tqdm(subjects.items()):
            for file in info["file"]:
                if "_08" in file:
                    ...
                    file = os.path.join(
                        ROOT_DIR, os.path.split(file)[1].replace("png", "jpg")
                    )
